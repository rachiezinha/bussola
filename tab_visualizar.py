...
- Proporção do total → **Pizza** ou **Barra Empilhada**
    ...
    if df_base is None or df_base.empty:
        st.warning("⚠️ Carregue um arquivo válido na aba **Carregar Dados**.")
        return
    ...
    if tipo_grafico == "Barras":
        ...
        if agg == "Soma":
            grp = df.groupby(eixo_x, dropna=False)[eixo_y].sum().reset_index()
        elif agg == "Contagem":
            grp = df.groupby(eixo_x, dropna=False)[eixo_y].count().reset_index()
        else:
            grp = df.groupby(eixo_x, dropna=False)[eixo_y].mean().reset_index()
        ...
        grp = df.groupby(eixo_x, dropna=False)[eixo_y].sum().reset_index().sort_values(eixo_x)
        ...
        grp_pz = df.groupby(col_pz_names, dropna=False)[col_pz_vals].sum().reset_index()
        grp_pz = grp_pz.sort_values(col_pz_vals, ascending=False).head(10)

        n_cat = len(grp_pz)
        if n_cat > 6:
            st.warning(
                f"⚠️ Há {n_cat} categorias — pizza fica difícil de ler com muitas fatias. "
                "Considere **Barras** ou **Ranking**."
            )
        ...
        fig = px.pie(
            ...
            color_discrete_sequence=[
                OURO, "#e8b84b", "#a67820", MARROM, NEUTRO,
        ...
        fig.update_traces(line_color=OURO, fillcolor="rgba(212,175,55,0.25)")
        ...
        seg_col = c2.selectbox(
            "Segmentação (cor)",
            [c for c in cols_cat if c != eixo_x],
            key="stack_seg",
        )
        eixo_y = c3.selectbox("Valor", cols_num, key="stack_y")

        grp = df.groupby([eixo_x, seg_col], dropna=False)[eixo_y].sum().reset_index()
        grp[eixo_y] = grp[eixo_y].fillna(0)
        ...
        if agg_r == "Soma":
            grp = df.groupby(col_cat, dropna=False)[col_val].sum().reset_index()
        elif agg_r == "Contagem":
            grp = df.groupby(col_cat, dropna=False)[col_val].count().reset_index()
        else:
            grp = df.groupby(col_cat, dropna=False)[col_val].mean().reset_index()

        grp = grp.dropna(subset=[col_val])
        grp = grp.sort_values(col_val, ascending=False).head(top_n)
        grp = grp.sort_values(col_val, ascending=True)
